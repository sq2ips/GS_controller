...
class MyApp(wx.Frame):
    ...
    def Update(self, event):
        ...
        if self.ser != None and self.ser.is_open:
            ...
            if data[0] != "STST":
                self.port=None
                self.ser=None
                wx.MessageBox("incorrect response from device. Please check if the correct serial port is selected.", "Error" ,wx.OK | wx.ICON_ERROR)
            else:
                if self.bpwait > 0:
                    self.bpwait-=1
                else:
    ...
    def tryInitSerial(self):
        sr = GetTty()
        if sr[1] == -1:
            wx.MessageBox("Unable to automatically detect the device. Configure it manually or check that it is connected and running", "Auto initialization error", wx.OK | wx.ICON_ERROR)
        else:
            try:
                self.ser = serial.Serial(f"/dev/{self.port}", timeout=3)
            except Exception as e:
                wx.MessageBox(str(e)+"\n"+"Unable to automatically detect the device. Configure it manually or check that it is connected and running", "Serial port initiation error" ,wx.OK | wx.ICON_ERROR)
